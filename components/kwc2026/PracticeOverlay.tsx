...
const PracticeOverlay = ({ open, mode, roundLabel, tricks, onClose }: Props) => {
  ...
  // 全部完成 → 自動結束並記錄用時
  useEffect(() => {
    if (running && tricks.length > 0 && doneCount === tricks.length) {
      setRunning(false)
      setFinished(true)
      setUsedMs(totalMs - remaining)
    }
  }, [doneCount, running, tricks.length, totalMs, remaining])
    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          {/* 計時與分數 */}
          <div className="shrink-0 px-4 py-4 text-center border-b border-gray-200 dark:border-neutral-800">
            <motion.p
            ...
            {finished && (
              <motion.p
                className="mt-3 text-sm font-medium text-gray-700 dark:text-neutral-300"
          ...
          <AnimatePresence>
            {showVideo && (
              <motion.div
                className="shrink-0 border-b border-gray-200 dark:border-neutral-800 bg-black"
              ...
              >
                ...
                <p className="text-[11px] text-center text-neutral-400 py-1">
